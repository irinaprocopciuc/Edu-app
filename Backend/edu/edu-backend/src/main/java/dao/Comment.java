    ...
    public List<Map<String,String>> getComments(String fileName, String courseName){
        try {
            if(new File(System.getProperty("user.dir")+"\\courseFiles\\"+courseName+"\\"+fileName).exists()){
                ...
                String query = "select c.commentId,c.userId,c.message,c.date,u.name from comment c join user u on c.userId = u.idUser where fileName=? and courseName = ?;";
                PreparedStatement stmt = conn.prepareStatement(query);
                ...
                while(rs.next()){
                    ...
                    comment.put("name",rs.getString(5));
